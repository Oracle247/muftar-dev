import { ProgessStatus, StatusTag } from "src/components/Badge";
import Table, { BodyProps } from "src/components/table/Table";
import { FormSelect } from "src/components/Input";

const OrganisationOperationsBrokerOrderSpecificNotifications = () => {
  const headers = [
  ...
  const dd2 = (): BodyProps[] => {
    return body2.map((data) => {
      const checkType = () => {
        if (data?.type === "Carrier Messages") {
          return (
            <StatusTag
              text="Carrir Messages"
              background="bg-muftar-blue-50"
              color="text-muftar-blue-700"
            />
          );
        }
        if (data?.type === "Customer Messages") {
          return (
            <StatusTag
              text="Customer Messages"
              background="bg-muftar-gray-50"
              color="text-muftar-grya-700"
            />
          );
        }
      };
...
export default OrganisationOperationsBrokerOrderSpecificNotifications;
