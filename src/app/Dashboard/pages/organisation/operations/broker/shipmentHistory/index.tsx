...
import { BrokerProgessStatus, ProgessStatus } from "src/components/Badge";

const OrganisationOperationsBrokerShipmentHistory = () => {
  const headers = [
  ...
  const dd = (): BodyProps[] => {
    return body.map((data) => {
      return {
        ...data,
        status: (
          <BrokerProgessStatus text={data?.status?.toLocaleLowerCase()} />
        ),
        actions: (
...
export default OrganisationOperationsBrokerShipmentHistory;
